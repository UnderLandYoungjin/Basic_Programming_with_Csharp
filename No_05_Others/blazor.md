...
4. 생성 후 NuGet 패키지 설치
   - 솔루션 탐색기 → 프로젝트 우클릭 → **NuGet 패키지 관리** → `Microsoft.Data.Sqlite` 검색 → 설치
...
```
BoardWeb/
├── Program.cs                  // 진입점 (한 줄만 추가)
├── Data/
│   ├── DB.cs                   // SQLite 연결 + 테이블 자동 생성
│   ├── Post.cs                 // 게시글 모델
│   └── PostDao.cs              // DB 접근 계층 (CRUD)
└── Components/
    ├── _Imports.razor          // 공용 using (한 줄만 추가)
    ├── Layout/
    │   └── NavMenu.razor       // 왼쪽 메뉴 (교체)
    └── Pages/
        ├── Board.razor         // 글 목록
        ├── PostView.razor      // 글 보기
        └── PostEdit.razor      // 글 쓰기·수정
```

계층은 도서 대여 시스템과 동일하게 나뉜다.
...
프로젝트에 `Data` 폴더를 만들고(프로젝트 우클릭 → 추가 → 새 폴더) 아래 세 파일을 추가한다.

### 5-1. Data/DB.cs — 연결 헬퍼 + 테이블 자동 생성
...
## 6. Program.cs — 한 줄만 추가

템플릿이 만들어 준 `Program.cs`에서 **`DB.Init();` 한 줄만 추가**한다. 전체는 아래와 같다.
...
## 7. 공용 using 등록 — Components/_Imports.razor

페이지마다 `@using BoardWeb.Data`를 쓰지 않도록 공용 파일에 한 줄 추가한다.
...
`.razor` 파일은 위쪽이 HTML(화면), 아래 `@code` 블록이 C#(로직)이다.
WinForms로 치면 디자이너 화면과 코드 비하인드가 한 파일에 같이 있는 셈이다.

페이지를 추가하려면: `Components/Pages` 우클릭 → 추가 → **Razor 구성 요소**.

### 8-1. Components/Pages/Board.razor — 글 목록
...
### 8-2. Components/Pages/PostView.razor — 글 보기
...
> **`prerender: false`를 붙인 이유**
> Blazor Server는 기본적으로 페이지를 두 번 그린다(서버에서 미리 한 번 + 연결 후 한 번).
> 그대로 두면 `OnInitialized`가 두 번 돌아서 조회수가 **2씩** 올라간다.
> `prerender: false`는 미리 그리기를 끄고 한 번만 실행하게 한다.

### 8-3. Components/Pages/PostEdit.razor — 글 쓰기·수정

새 글 작성과 기존 글 수정을 한 페이지로 처리한다.
주소에 Id가 있으면 수정, 없으면 새 글이다.
...
## 9. 왼쪽 메뉴 — Components/Layout/NavMenu.razor

템플릿의 `NavMenu.razor` 내용을 전부 지우고 아래로 교체한다.
...
F5 (또는 `dotnet run` 후 표시되는 주소로 접속)

1. 브라우저가 열리면 빈 게시판이 보인다. 프로젝트 폴더에 `board.db`가 생긴 것도 확인해 보자.
...
| 증상 | 원인 | 해결 |
|------|------|------|
| 실행하자마자 라우트 오류 (`/`가 중복) | `Home.razor`를 안 지움 | `Components/Pages/Home.razor` 삭제 |
| 버튼을 눌러도 아무 반응이 없음 | 페이지에 `@rendermode InteractiveServer`가 빠짐 | PostView·PostEdit 맨 위 지시문 확인 |
| `Microsoft.Data.Sqlite`를 찾을 수 없음 | NuGet 패키지 미설치 | NuGet에서 `Microsoft.Data.Sqlite` 설치 |
| `PostDao`를 찾을 수 없음 | `_Imports.razor`에 using 누락 | `@using BoardWeb.Data` 추가 |
| 조회수가 2씩 올라감 | 사전 렌더링으로 초기화가 두 번 실행 | PostView의 `prerender: false` 지시문 확인 |
| 데이터를 초기화하고 싶음 | — | 앱 종료 후 프로젝트 폴더의 `board.db` 삭제 |
