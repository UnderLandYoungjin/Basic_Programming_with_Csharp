...
| `StackPanel` | 위에서 아래 또는 왼쪽에서 오른쪽으로 쌓아서 배치 | 버튼 목록, 메뉴 구성 |
| `DockPanel` | 상/하/좌/우 가장자리에 붙여서 배치 | 메뉴바, 상태바, 사이드바 구성 |

---

## 시작 전 주의사항 — 프로젝트 이름 확인

> 이 강의의 모든 예제는 **1강에서 만든 `WpfHelloApp` 프로젝트를 그대로 사용**합니다.
> 코드 안의 `namespace WpfHelloApp` 이 **본인 프로젝트 이름과 반드시 일치**해야 합니다.
>
> 프로젝트 이름 확인 방법: Visual Studio 상단 `솔루션 탐색기` → 프로젝트 폴더 이름 확인

```
솔루션 탐색기
└── WpfHelloApp          <- 이 이름을 namespace에 그대로 사용
    ├── App.xaml
    ├── MainWindow.xaml
    └── MainWindow.xaml.cs
```
...
## 1. Grid — 행과 열로 나누는 레이아웃

`Grid`는 화면을 **표(행 x 열)** 처럼 나눠서 각 칸에 컨트롤을 배치합니다.
...
**목표:** 2행 x 2열 Grid를 만들고 각 칸에 버튼을 배치합니다.
각 버튼을 클릭하면 어느 위치의 버튼인지 메시지가 나타납니다.

#### MainWindow.xaml — 전체 코드로 교체
...
#### MainWindow.xaml.cs — 전체 코드로 교체

```csharp
using System.Windows;

// namespace 이름이 본인 프로젝트 이름과 일치하는지 반드시 확인!
// 솔루션 탐색기에서 프로젝트 폴더 이름을 확인하세요.
...
──────────────────────────────        ────────────────────────────────────
<Button Click="b00_Click"/>  ──────► private void b00_Click(...) { }
<Button Click="b01_Click"/>  ──────► private void b01_Click(...) { }
<Button Click="b10_Click"/>  ──────► private void b10_Click(...) { }
<Button Click="b11_Click"/>  ──────► private void b11_Click(...) { }
```
...
## 2. StackPanel — 순서대로 쌓는 레이아웃

`StackPanel`은 자식 컨트롤을 **위에서 아래(세로)** 또는 **왼쪽에서 오른쪽(가로)** 방향으로 순서대로 나열합니다.
...
<Window x:Class="WpfHelloApp.MainWindow"
    ...
    <StackPanel Orientation="Vertical"
        ...
        <!-- Margin="0,0,0,8": 버튼 아래쪽에 8px 여백 (버튼 간 간격) -->
...
<Window x:Class="WpfHelloApp.MainWindow"
        xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
        Title="가로 StackPanel" Height="150" Width="300">
    ...
    <StackPanel Orientation="Horizontal"
        ...
        <Button Content="이전" Width="80" Height="35"
                Margin="0,0,8,0"
                Click="bPrev_Click"/>
...
namespace WpfHelloApp
{
    public partial class MainWindow : Window
    {
        ...
        {
            MessageBox.Show("이전 버튼 클릭");
        }

        private void bNext_Click(object sender, RoutedEventArgs e)
        {
            MessageBox.Show("다음 버튼 클릭");
        }
    }
}
```
...
**목표:** 상단 메뉴바 + 하단 상태바 + 좌측 사이드바 + 중앙 본문으로 구성된
...
<Window x:Class="WpfHelloApp.MainWindow"
    ...
    <!-- LastChildFill="True": 마지막 자식이 남은 중앙 공간을 모두 채움 -->
    <DockPanel LastChildFill="True">
        ...
        <!-- Dock 지정 없는 마지막 자식: 남은 중앙 공간을 전부 채움 -->
...
namespace WpfHelloApp
{
    public partial class MainWindow : Window
    {
        ...
        // 상단 메뉴바 버튼 클릭 시 실행 — XAML Click="bTop_Click" 과 연결
        private void bTop_Click(object sender, RoutedEventArgs e)
        {
            MessageBox.Show("상단 메뉴바 영역입니다.");
        }

        // 하단 상태바 버튼 클릭 시 실행 — XAML Click="bBottom_Click" 과 연결
        private void bBottom_Click(object sender, RoutedEventArgs e)
        {
            MessageBox.Show("하단 상태바 영역입니다.");
        }

        // 왼쪽 사이드바 버튼 클릭 시 실행 — XAML Click="bLeft_Click" 과 연결
        private void bLeft_Click(object sender, RoutedEventArgs e)
        {
            MessageBox.Show("왼쪽 사이드바 영역입니다.");
        }

        // 중앙 본문 버튼 클릭 시 실행 — XAML Click="bCenter_Click" 과 연결
...
**목표:** Grid로 전체 구조를 나누고, 하단에 StackPanel로 확인/취소 버튼을
가로로 배치합니다. 확인 클릭 시 "저장되었습니다!", 취소 클릭 시 창이 닫힙니다.
...
<Window x:Class="WpfHelloApp.MainWindow"
    ...
    <Grid Margin="10">
        ...
        <!-- 1행: Grid.Row="1" 로 1번 행에 배치 -->
        <!-- StackPanel로 버튼 2개를 가로로 배치 -->
        <!-- Orientation="Horizontal": 왼쪽에서 오른쪽으로 나열 -->
        ...
        <StackPanel Grid.Row="1"
            ...
            <!-- 확인 버튼: Margin 오른쪽 8px로 취소 버튼과 간격 확보 -->
            <Button Content="확인"
                    Width="90" Height="35"
                    Margin="0,0,8,0"
                    Click="bOk_Click"/>
...
namespace WpfHelloApp
{
    public partial class MainWindow : Window
    {
        ...
        // 확인 버튼 클릭 시 실행 — XAML Click="bOk_Click" 과 연결
        private void bOk_Click(object sender, RoutedEventArgs e)
        {
            MessageBox.Show("저장되었습니다!");  // 저장 완료 메시지 출력
        }

        // 취소 버튼 클릭 시 실행 — XAML Click="bCancel_Click" 과 연결
        // this.Close(): 현재 창을 닫아서 프로그램을 종료함
        private void bCancel_Click(object sender, RoutedEventArgs e)
        {
            this.Close();               // 창 닫기 -> 프로그램 종료
        }
    }
}
```
...
- **`namespace` 이름은 반드시 본인 프로젝트 이름과 일치**해야 합니다.
...
`Vertical`(기본값)은 위에서 아래, `Horizontal`은 왼쪽에서 오른쪽 방향입니다.
...
<Window x:Class="WpfHelloApp.MainWindow"
        xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
        Title="문제 5" Height="250" Width="300">
    <Grid>
        <Grid.RowDefinitions>
            <RowDefinition Height="*"/>
            <RowDefinition Height="*"/>
        ...
        <Button Grid.Row="__" Content="0번 행 버튼" Margin="10" Click="b0_Click"/>
        <Button Grid.Row="__" Content="1번 행 버튼" Margin="10" Click="b1_Click"/>
        <Button Grid.Row="__" Content="2번 행 버튼" Margin="10" Click="b2_Click"/>
    </Grid>
</Window>
```
...
```xml
<Button Grid.Row="0" Content="0번 행 버튼" Margin="10" Click="b0_Click"/>
<Button Grid.Row="1" Content="1번 행 버튼" Margin="10" Click="b1_Click"/>
<Button Grid.Row="2" Content="2번 행 버튼" Margin="10" Click="b2_Click"/>
```
